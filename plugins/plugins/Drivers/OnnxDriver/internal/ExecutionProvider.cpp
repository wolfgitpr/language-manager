...
namespace LangPlugins::onnxDriver
{
    ...
    bool initCUDA(const Ort::SessionOptions &options, const int deviceIndex, std::string *errorMessage) {
        ...
        const Ort::Status createStatus(ortApi.CreateCUDAProviderOptions(&cudaOptionsPtr));
        ...
        // goes out of scope.
        const std::unique_ptr<OrtCUDAProviderOptionsV2, decltype(ortApi.ReleaseCUDAProviderOptions)> cudaOptions(
        ...
        // The following block of code sets device_id
        {
            ...
            const auto cudaDeviceIdStr = std::to_string(deviceIndex);
            const auto cudaDeviceIdCStr = cudaDeviceIdStr.c_str();
            ...
            const Ort::Status updateStatus(ortApi.UpdateCUDAProviderOptions(cudaOptions.get(), cudaOptionsKeys,
    ...
    bool initDirectML(Ort::SessionOptions &options, const int deviceIndex, std::string *errorMessage) {
