...
namespace LangPlugins::onnxDriver
{
    ...
    public:
        ...
        static Ort::Value createOrtValueFromTensor(const LangCore::NO<ITensor> &tensor,
                                                   const Ort::MemoryInfo &memoryInfo,
                                                   LangCore::Error *error = nullptr) {
            ...
            default:
                if (error) {
                    *error = {LangCore::Error::InvalidArgument, "Unsupported data type"};
                }
                return Ort::Value(nullptr);
            }
        }

        static LangCore::NO<ITensor> createTensorFromOrtValue(const Ort::Value &ortValue,
                                                              LangCore::Error *error = nullptr) {
            if (!ortValue.IsTensor()) {
                if (error) {
                    *error = {LangCore::Error::InvalidArgument, "Ort::Value is not a tensor"};
                }
                return {};
            }
            ...
            default:
                if (error) {
                    *error = {LangCore::Error::InvalidArgument, "Unsupported ONNX tensor element type"};
                }
                return {};
            }
        ...
        sessionRun(const LangCore::NO<Api::Onnx::L1::SessionStartInput> &sessionStartInput,
            ...
            auto result = LangCore::NO<Api::Onnx::L1::SessionResult>::create();
            try {
    ...
    LangCore::Error
    Session::Impl::validateInputValueMap(const LangCore::NO<Api::Onnx::L1::SessionStartInput> &input) const {
        const auto &inputValueMap = input->inputs;
        if (inputValueMap.empty()) {
            return {LangCore::Error::SessionError, "Input map is empty"};
        }
        ...
        // message.
        {
            ...
            if (flagMissing || flagExtra) {
                return {LangCore::Error::SessionError, msgStream.str()};
            }
        }
        return {}; // no error
    }
    ...
    LangCore::Expected<void> Session::open(const fs::path &path,
                                           const LangCore::NO<Api::Onnx::L1::SessionOpenArgs> &args) {
        __stdc_impl_t;

        if (isOpen()) {
            Log.langCoreWarning("Session - Session %1 is already open!", path.string());
            return LangCore::Error(LangCore::Error::SessionError, "session is already open");
        }

        // Open
        Log.langCoreDebug("Session - Try open " + path.string());
        if (!fs::is_regular_file(path)) {
            return LangCore::Error(LangCore::Error::FileNotOpen, "not a regular file");
        }
        ...
        // Calculate hash
        {
            ...
            if (!getFileInfo(canonical_path, hash, hash_str, size)) {
                return LangCore::Error(LangCore::Error::FileNotOpen, "failed to read file");
            }
    ...
    LangCore::Expected<void> Session::close() {
        __stdc_impl_t;

        if (!impl.group)
            return LangCore::Error(LangCore::Error::SessionError, "session is not open");
    ...
    Session::run(const LangCore::NO<LangCore::TaskStartInput> &input) {
        __stdc_impl_t;
