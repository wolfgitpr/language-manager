...
namespace LangPlugins
{
    ...
    LangCore::Expected<void> verify(const ITensor::DataType dataType, const std::vector<int64_t> &shape,
                                    const size_t dataSize) {
    ...
    LangCore::Expected<LangCore::NO<Tensor>> Tensor::create(const DataType dataType,
                                                            const std::vector<int64_t> &shape) {
        const auto maybeTotalElements = getElementCountFromShape(dataType, shape);
        if (!maybeTotalElements.has_value())
        ...
        auto tensor = LangCore::NO<Tensor>::create();
    ...
    LangCore::Expected<LangCore::NO<Tensor>>
    Tensor::createFromRawData(const DataType dataType, const std::vector<int64_t> &shape, const Container &data) {
        auto tensor = LangCore::NO<Tensor>::create();
    ...
    LangCore::Expected<LangCore::NO<Tensor>> Tensor::createFromRawView(const DataType dataType,
                                                                       const std::vector<int64_t> &shape,
    ...
    LangCore::Expected<LangCore::NO<Tensor>>
    Tensor::createFromRawData(const DataType dataType, const std::vector<int64_t> &shape, Container &&data) {
        auto tensor = LangCore::NO<Tensor>::create();
        if (auto exp = verify(dataType, shape, data.size()); !exp)
            return exp.takeError();
    ...
    LangCore::NO<ITensor> Tensor::clone() const {
        auto tensor = LangCore::NO<Tensor>::create();
