        ...
        for (size_t i = 0; i < batchSize; ++i) {
            ...
            if (phonemes.empty()) {
                g2pResult->g2pResult.emplace_back(LangCore::G2pRes{
                    std::string(lyric), std::string("eng"), std::string(lyric), std::vector<std::string>(),
                    std::string("copy"), LangCore::PhonemeGenerationFailed});
            } else {
                g2pResult->g2pResult.emplace_back(LangCore::G2pRes{std::string(lyric), std::string("eng"),
