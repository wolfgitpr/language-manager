...
namespace LangPlugins::InferUtil
{
    ...
    class TensorHelper {
    public:
        static LangCore::Expected<TensorHelper> createFor1DArray(size_t size) {
            TensorHelper helper;
            ...
            if (STDCORELIB_UNLIKELY(dataPtr == nullptr)) {
                return LangCore::Error(LangCore::Error::SessionError, "failed to create tensor");
            }
        ...
        LangCore::NO<Tensor> &value() { return _tensor; }

        LangCore::NO<Tensor> &&take() { return std::move(_tensor); }
