...
namespace LangPlugins::InferUtil
{
    ...
    inline void ConfigurationParser::parse_tagger_required(std::vector<Api::TemplateTagger::L1::TaggerUtilEntry> &out,
                                                           const std::string &fieldName) {
        const auto &config = *pConfig;
        ...
        if (const auto it = config.find(fieldName); it != config.end()) {
            if (!it->second.isArray()) {
                ...
                for (size_t i = 0; i < arr.size(); ++i) {
                    ...
                    const auto &obj = item.toObject();
                    Api::TemplateTagger::L1::TaggerUtilEntry entry;

                    if (const auto typeIt = obj.find("type"); typeIt != obj.end()) {
                        if (typeIt->second.isString()) {
                            entry.type = typeIt->second.toString();
                        } else {
                            collectError("tagger entry #" + std::to_string(i) + " field \"type\" must be string");
                            continue;
                        }
                    } else {
                        collectError("tagger entry #" + std::to_string(i) + " missing required field \"type\"");
                        continue;
                    }
                    ...
                    if (const auto modeIt = obj.find("discard"); modeIt != obj.end()) {
                        ...
                        collectError("tagger entry #" + std::to_string(i) + " missing required field \"mode\"");
                        continue;
                    }
                    out.push_back(std::move(entry));
                }
