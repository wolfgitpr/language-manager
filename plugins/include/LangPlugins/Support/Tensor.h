#ifndef LANGPLUGINS_TENSOR_H
#define LANGPLUGINS_TENSOR_H
...
namespace LangPlugins
{
    ...
    class ITensor : public LangCore::NamedObject {
    public:
        ...
        virtual LangCore::NO<ITensor> clone() const = 0;
    };
...
#define LANGPLUGINS_TENSOR_REGISTER_DATATYPE(_CppType, _EnumType)                                                      \
    ...
    LANGPLUGINS_TENSOR_REGISTER_DATATYPE(float, Float)
    LANGPLUGINS_TENSOR_REGISTER_DATATYPE(int64_t, Int64)
    LANGPLUGINS_TENSOR_REGISTER_DATATYPE(bool, Bool)

#undef LANGPLUGINS_TENSOR_REGISTER_DATATYPE
    ...
    class LANGPLUGINS_EXPORT Tensor : public ITensor {
    public:
        static constexpr size_t ALIGNMENT = sizeof(int64_t);

        template <typename T>
        using AlignedVector = std::vector<T, LangPlugins::AlignedAllocator<T, ALIGNMENT>>;
        ...
        static LangCore::Expected<LangCore::NO<Tensor>> create(DataType dataType, const std::vector<int64_t> &shape);
        ...
        static LangCore::Expected<LangCore::NO<Tensor>>
        ...
        /// \post On success, the returned tensor owns its own copy of the data.
        ///       The original data is not modified.
        static LangCore::Expected<LangCore::NO<Tensor>> createFromRawView(DataType dataType,
        ...
        ///       The original data is not modified.
        template <typename T>
        static LangCore::Expected<LangCore::NO<Tensor>> createFromView(const std::vector<int64_t> &shape,
        ...
        template <typename T>
        static LangCore::Expected<LangCore::NO<Tensor>> createScalar(T value, bool zeroDimensions = false);
        ...
        ///         On failure: An error describing the cause of the failure.
        template <typename T>
        static LangCore::Expected<LangCore::NO<Tensor>> createFilled(const std::vector<int64_t> &shape, T value);
        ...
        LangCore::NO<ITensor> clone() const override;
    ...
    ////////////////////////////////////////////////////////////////////////////////
    template <typename T>
    LangCore::Expected<LangCore::NO<Tensor>> Tensor::createFromView(const std::vector<int64_t> &shape,
                                                                    const stdc::array_view<T> &data) {
        static_assert(tensor_traits<T>::is_valid, "Unsupported tensor data type");
        static_assert(!std::is_same_v<T, bool> || sizeof(bool) == 1, "sizeof(bool) == 1 does not satisfy");
    ...
    template <typename T>
    LangCore::Expected<LangCore::NO<Tensor>> Tensor::createScalar(T value, const bool zeroDimensions) {
        static_assert(tensor_traits<T>::is_valid, "Unsupported tensor data type");
        static_assert(!std::is_same_v<T, bool> || sizeof(bool) == 1, "sizeof(bool) == 1 does not satisfy");
    ...
    template <typename T>
    LangCore::Expected<LangCore::NO<Tensor>> Tensor::createFilled(const std::vector<int64_t> &shape, T value) {
