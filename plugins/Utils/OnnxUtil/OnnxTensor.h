...
namespace LangPlugins
{
    ...
    public:
        ...
        static LangCore::Expected<LangCore::NO<OnnxTensor>> create(DataType, const std::vector<int64_t> &shape);
         ...
         */
        static LangCore::Expected<LangCore::NO<OnnxTensor>> createFromRawView(DataType dataType,
                                                                              const std::vector<int64_t> &shape,
                                                                              const stdc::array_view<std::byte> &data);

        template <typename T>
        static LangCore::Expected<LangCore::NO<OnnxTensor>> createFromView(const std::vector<int64_t> &shape,
                                                                           const stdc::array_view<T> &data);

        template <typename T>
        static LangCore::Expected<LangCore::NO<OnnxTensor>> createScalar(T value, bool zeroDimensions = false);

        template <typename T>
        static LangCore::Expected<LangCore::NO<OnnxTensor>> createFilled(const std::vector<int64_t> &shape, T value);
        /**
         ...
         */
        static LangCore::Expected<LangCore::NO<OnnxTensor>> createFromOrtValue(Ort::Value &&value);

        static LangCore::Expected<LangCore::NO<OnnxTensor>> createFromTensor(const LangCore::NO<ITensor> &tensor);
    ...
    template <typename T>
    LangCore::Expected<LangCore::NO<OnnxTensor>> OnnxTensor::createFromView(const std::vector<int64_t> &shape,
                                                                            const stdc::array_view<T> &data) {
    ...
    template <typename T>
    LangCore::Expected<LangCore::NO<OnnxTensor>> OnnxTensor::createScalar(T value, const bool zeroDimensions) {
        static_assert(tensor_traits<T>::is_valid, "Unsupported tensor data type");
        static_assert(!std::is_same_v<T, bool> || sizeof(bool) == 1, "sizeof(bool) == 1 does not satisfy");
    ...
    template <typename T>
    LangCore::Expected<LangCore::NO<OnnxTensor>> OnnxTensor::createFilled(const std::vector<int64_t> &shape, T value) {
