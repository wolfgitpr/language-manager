...
macro(_cur_add_library_internal _target _type)
    ...
    if (_IS_PLUGIN)
        ...
        if (_link_libs)
            ...
            function(_add_dep_copy _lib)
                if (${_lib} IN_LIST _processed_libs)
                    return()
                endif ()
                list(APPEND _processed_libs ${_lib})
                if (TARGET "${_lib}")
                    ...
                    if (_lib_type STREQUAL "SHARED_LIBRARY" OR _lib_type STREQUAL "MODULE_LIBRARY")
                        ...
                        get_target_property(_lib_link_libs ${_lib} LINK_LIBRARIES)
                        if (_lib_link_libs)
                            foreach (_sub_lib IN LISTS _lib_link_libs)
                                _add_dep_copy(${_sub_lib})
                            endforeach ()
                        endif ()
                    endif ()
                endif ()
            ...
            if (_copy_deps_commands)
                add_custom_command(TARGET ${_target} POST_BUILD ${_copy_deps_commands}
                        COMMENT "Copying dependent shared libraries (including one level of indirect) to plugin directory")
            endif ()
        endif ()
    endif ()
