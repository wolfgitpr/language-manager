...
namespace LangMgr
{
    struct LANGMGR_EXPORT DependencyRequirement {
    ...
    struct LANGMGR_EXPORT ResolvedDependency {
        std::string packageId;
        std::string moduleId;
        int level = -1;
        std::string version;
    ...
    struct LANGMGR_EXPORT ModuleMetadata {
        std::string packageId;
        std::string moduleId;
        std::string type;
        std::string iid;
    ...
    struct LANGMGR_EXPORT VersionSelectionResult {
        bool success = false;
    ...
    struct LANGMGR_EXPORT PackageInitializationPlan {
        std::string packageId;
        std::filesystem::path packagePath;
    ...
    class LANGMGR_EXPORT DependencyGraph {
    public:
        DependencyGraph();
        ~DependencyGraph();
    ...
    void LANGMGR_EXPORT printModule(const ModuleMetadata &module);
    void LANGMGR_EXPORT printDependency(const ResolvedDependency &dep);
    void LANGMGR_EXPORT printModuleList(const std::vector<ModuleMetadata> &modules, const std::string &title = "");
    void LANGMGR_EXPORT printPackageInitOrder(const std::vector<PackageInitializationPlan> &packageOrder,
