...
namespace LangPlugins
{
    ...
    LangMgr::Expected<void> verify(const ITensor::DataType dataType, const std::vector<int64_t> &shape,
                                   const size_t dataSize) {
    ...
    LangMgr::Expected<LangMgr::NO<Tensor>> Tensor::create(const DataType dataType, const std::vector<int64_t> &shape) {
        const auto maybeTotalElements = getElementCountFromShape(dataType, shape);
        if (!maybeTotalElements.has_value())
    ...
    LangMgr::Expected<LangMgr::NO<Tensor>> Tensor::createFromRawView(const DataType dataType,
                                                                     const std::vector<int64_t> &shape,
